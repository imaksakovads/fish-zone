# e46ecee3-1e22-4fb0-885c-19e5f6dde469 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `drop-shot-table.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `drop-shot-table.html`
...
- A single self-contained `drop-shot-table.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `drop-shot-table.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `drop-shot-table.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
